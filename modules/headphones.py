...
class Headphones(object):
    ...
    def fetch(self, command, url=None, api_key=None):
        ...
        try:
            self.logger.info('calling headphones @ %s' % url)
            ...
            self.logger.debug('headphones ping response body: %s' % body_text)

            return loads(body_text)
        except Exception as e:
            self.logger.error("Unable to contact headphones via %s: %s" % (url, e))
